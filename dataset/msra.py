...
class MSRA:
    ...
    def __init__(self, data_set_path, batch_size, shuffle=True):
        self._image_files = path.list_all_images(data_set_path)
...
class MSRASequence(Sequence):
    ...
    def __init__(self, msra_data_path, batch_size, shuffle=True, multithread=None):
        ...
        self._image_paths = path.list_all_images(msra_data_path)
...
class MSRATrainValidationSplitter(TrainValidationSplitter):
    def __init__(self, msra_path, validation_percentage):
        ...
        self._msra_path = path.absolute_path(msra_path)

    def _split_training_data(self, train_dir, val_dir, validation_percentage):
        image_names = path.list_all_images(self._msra_path)
        ...
        for i in range(total_images):
            ...
            path.symlink(path.join_path(link_dir, img_name),
                         path.join_path(self._msra_path, img_name))
            path.symlink(path.join_path(link_dir, gt_name),
                         path.join_path(self._msra_path, gt_name))


def _gt_file(img_file):
    img_name = path.get_file_name(img_file, False)
    return f'{img_name}.gt'


def _full_path(msra_path, file_name):
    return path.join_path(msra_path, file_name)
